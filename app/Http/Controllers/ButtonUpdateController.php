...
class ButtonUpdateController extends Controller
{
    ...
    public function execute(Request $request)
    {
        ...
        $result = DB::connection($request->database)->transaction(function () use ($request) {
            ...
            DB::connection($request->database)->select(DB::raw('SELECT ps.StationNo, ps.DivNo, ps.ScreenName, ps.ScreenEnable INTO #tempState FROM PosScreens ps'));
            DB::connection($request->database)->select(DB::raw('SELECT DISTINCT ps.StationNo, divs.DivNo, ps.ScreenID, ps.ScreenName, ps.ModuleID INTO #tempScreens FROM PosScreens ps, Divisions divs WHERE ps.StationNo = 0 AND ps.DivNo = 0'));
